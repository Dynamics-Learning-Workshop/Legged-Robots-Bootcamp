...
walker.control.P = 0.1; walker.N = 4;
...
% X0 = [T_opt u_opt];
A = []; B = []; Aeq = []; Beq = [];
LB = [time_min z_ss_lb z_bfs_lb u_lb];
UB = [time_max z_ss_ub z_bfs_ub u_ub];
% %NONLCON = @(x)constraints(x,z0,N,D);
%'sqp'
OPTIONS = optimoptions('fmincon','Display','iter','Algorithm','interior-point','OptimalityTolerance',1e-8);
